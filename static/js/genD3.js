...
//console.log(x.domain)
    ...
    var mouseover_daily = function (r_data) {
        ...
        d3.csv("../static/data/Call_all_years_daily.csv").then(function (data) {
            ...
            let data_y = data.filter(function (d) {
                return d.call_year == clicked_year;
            })
            let data_m = data_y.filter(function (d) {
                return d.call_month == clicked_month;
            })
//console.log(data_m);
//console.log(data_m);
...
// Define the axes
            ...
            var month_line = d3.line()
                .x(function (d) {
                    return x1(d["SR CREATE DATE"]);
                })
                .y(function (d) {
                    return y1(d["call_count"]);
                });

            x1.domain([
                d3.min(data_m, function (d) {
                    return d["SR CREATE DATE"];
                }),
                d3.max(data_m, function (d) {
                    return d["SR CREATE DATE"];
                })
            ]);

            y1.domain([0, d3.max(data_m, function (d) {
                return d.call_count;
            })]).nice();
...
//console.log(data);
                    ...
                    data_y = data.filter(function (d) {
                        return d.call_year == clicked_year;
                    })
                    data_m = data_y.filter(function (d) {
                        return d.call_month == clicked_month;
                    })
                    let data_d = data_m.filter(function (d) {
                        return d.call_day == clicked_day;
                    })
                    ...
                    let sr_type = data_d.map(function (element) {
                        return (element["SR TYPE"]);
                    })
                    //console.log(sr_type);
                    y3.domain(sr_type);
                    let counts = data_d.map(function (element) {
                        return (element.count_sr_type);
                    })
                    x3.domain([0, d3.max(data_d, function (d) {
                        return d.count_sr_type;
                    })])
